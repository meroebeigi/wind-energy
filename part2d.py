...
# Rotor disc area
A_rotor = np.pi * R_rotor**2
x, y = np.meshgrid(np.arange(1, N_turbine + 1) * dx, np.arange(1, M_turbine + 1) * dy)
xs = x.flatten()
ys = y.flatten()
# Calculating the overlap area
ovl = overlap(N_turbine, M_turbine, R_rotor, k, xs, ys)
...
plt.figure(3)
plt.scatter(xs, ys, 2000 * abs(zita), zita, c=zita, cmap='autumn', edgecolors='k', linewidths=0.5)
plt.colorbar(label='Gain')
plt.axis('equal')
plt.grid(True)

# Plotting the Optimized induction factor in south-east wind direction
plt.figure(4)
plt.scatter(xs, ys, 50, qs, c=qs, cmap='summer', edgecolors='k', linewidths=0.5)
